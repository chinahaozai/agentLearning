...
这是 [端侧 AI 学习轨道](02-on-device-ai-track.md) 的**收官项目**，地位等同于主线的 [AgentOps Mini Platform](14-agentops-mini-platform-design.md)，但目标不同：
...
2. 走本地：Kotlin agent loop 调 **JNI→llama.cpp** 端侧推理，按需调**本地工具**或查**本地 RAG**。【[端侧:Part5.6](02-on-device-ai-track.md) + [C++:Part5](03-cpp-for-on-device-ai.md)】
3. 走云端：Retrofit 调**精简版 AgentOps** 服务，云端跑 agent loop + 大模型。【复用 [AgentOps](14-agentops-mini-platform-design.md)】
4. 每一步埋 **trace**，且端侧步骤额外记**设备指标**（首 token 延迟/吞吐/峰值内存/功耗/后端/路由）。【[第3篇] + [端侧:Part5.5](02-on-device-ai-track.md)】
...
**别重做整个 AgentOps**。一个 FastAPI 服务，提供一个 `POST /agent/run`：收到任务 → 跑云端 agent loop（大模型 + 工具）→ 返回结果 + trace 摘要。复用 [AgentOps 蓝图](14-agentops-mini-platform-design.md) 的 agent/trace 代码，砍掉异步队列/前端等重组件。
...
**端侧轨道**
1. [端侧 AI 学习轨道](02-on-device-ai-track.md) —— 推理/量化/NPU/profiling/端云协同
2. [C++ 补齐计划](03-cpp-for-on-device-ai.md) —— JNI、llama.cpp 集成

**主线（云半部 + Agent 基础）**
3. [LLM API 与 Tool Calling](10-llm-api-and-tool-calling.md) —— agent loop、工具调用
4. [Agent Trace 与可观测性](11-agent-trace-and-observability.md) —— trace/span 数据结构
5. [Agent Eval：评测与回归](12-agent-eval-and-metrics.md) —— 评测集、指标、对比
6. [Docker 部署 Agent 服务](13-docker-deploy-agent-service.md) —— 云半部容器化
7. [AgentOps Mini Platform](14-agentops-mini-platform-design.md) —— 云半部直接复用它的精简版
