...
这是 [端侧 AI 学习轨道](02-on-device-ai-track.md) **Part 5.5 的展开篇**，端侧展开篇系列第 2 篇，接上一篇 [端侧推理实战](05-on-device-inference-llamacpp.md)（那篇 Part 6 只做了轻量 benchmark，这篇把"测"讲透）。

**为什么这篇对你最值钱**：profiling 是你相对纯算法背景的人**最大的差异化**。回扣 [轨道 Part 1 / 3.4](02-on-device-ai-track.md)——端侧的瓶颈是内存带宽和功耗，纯算法的人对这些没感觉，**你天天用 Profiler / Perfetto / batterystats 排查 App，只是从没把它们对准过推理**。把这套迁移过来，就是简历和面试白板上别人画不出来的东西。
...
为什么 decode 是内存带宽瓶颈：每生成一个 token，都要把**整个模型权重**从内存搬一遍过一遍。所以 decode 速度几乎正比于"内存带宽 ÷ 模型大小"——这正是 [轨道 Part 3.4](02-on-device-ai-track.md) 说的，也是**量化能直接提速**的原因（搬的字节少了）。
...
profiling 的终点是**可展示的产出物**，直接喂给 [PocketAgent 收官项目](04-edge-cloud-agent-capstone.md) 的 M5 和它的"设备指标 trace"字段（[蓝图 Part 4](04-edge-cloud-agent-capstone.md)）。
...
**回到轨道**
- [端侧 AI 学习轨道](02-on-device-ai-track.md)（Part 5.5 全景）
- [端侧推理实战](05-on-device-inference-llamacpp.md)（上一篇，跑通 + 基础 benchmark）
- [PocketAgent 收官蓝图](04-edge-cloud-agent-capstone.md)（设备指标 trace / M5）
