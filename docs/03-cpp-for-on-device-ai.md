...
这是 [端侧 AI 学习轨道](02-on-device-ai-track.md) 的配套技能篇。真实端侧 JD 里 C++ 几乎是硬门槛（6 份里 4 份要求"精通 C++：内存管理 / 多线程 / 性能分析"），**这是你从 Android 转端侧最大的、也最该优先补的缺口**。
...
> **看懂这 8 步，你就看懂了所有端侧 LLM 引擎**。这跟主线 [第2篇](10-llm-api-and-tool-calling.md) 的 agentic loop 是同构的——只不过这次"调模型"发生在本机 native 层，而不是 HTTP。
...
**别按教科书顺序从头学 C++**，按"能跑、能用"的里程碑推进，和 [端侧轨道](02-on-device-ai-track.md) 的 M2 并行。
